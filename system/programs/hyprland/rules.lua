...
hl.window_rule({ match = { class = "chrome-nngceckbapebfimnlniiiahkandclblb-Profile_2" }, float = true })
...
hl.window_rule({ match = { title = "(?i)^(Picture-in-Picture)$" }, pin = true })
hl.window_rule({ match = { title = "(?i)^(Picture-in-Picture)$" }, size = "25% 25%" })
hl.window_rule({
...
-- browser(-based)
hl.window_rule({ match = { class = "^(zen|helium|firefox|chromium-browser|chrome-.*)$" }, scroll_touchpad = 0.1 })
